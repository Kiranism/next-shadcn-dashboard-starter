...
export async function GET(request: NextRequest) {
  try {
    ...
    const expirationResult = await processBonusExpiration(mockTransactions, mockUsers);
    ...
    if (expirationResult.expirationTransactions.length > 0) {
      ...
      console.log(`💸 Обработано ${expirationResult.summary.totalExpiredBonuses} истекших бонусов на сумму ${expirationResult.summary.totalExpiredAmount}`);
    }
    ...
    for (const notification of [...expirationResult.notifications, ...warningNotifications]) {
    ...
    const report = {
      ...
      notifications: {
        ...
        totalSent: expirationResult.notifications.length + warningNotifications.length,
      },
...
async function sendEmailNotification(notification: any): Promise<void> {
  ...
  console.log(`📧 Отправка email уведомления пользователю ${notification.userId}:`, {
