...
- **Статус**: ✅ Завершена
- **Приоритет**: 🔴 Высокий
- **Описание**: `api/admin/*` и часть `api/projects/*` не защищены. Нужна проверка Clerk и ролей.
- **Техническая сложность**: 3
- **Затраченное время**: [0] часов
- **Шаги выполнения**:
...
- **Затраченное время**: [0] часов
- **Шаги выполнения**:
  - [ ] Реализовать адаптер (Upstash Redis) и конфиг через .env
  - [ ] Graceful fallback на in-memory в dev
  - [ ] Мониторинг метрик лимитера
...
- **Затраченное время**: [0] часов
- **Шаги выполнения**:
  - [ ] Применить `validateWithSchema` в `webhook`, `projects/[id]/users` (POST/PUT) и др.
...
- **Затраченное время**: [0] часов
- **Шаги выполнения**:
  - [ ] Добавить CSP (script-src, img-src, connect-src и т.д.)
  - [ ] Добавить Referrer-Policy и Permissions-Policy
  - [ ] Проверить совместимость с Vercel/NGINX
...
- **Затраченное время**: [0] часов
- **Шаги выполнения**:
  - [ ] Перейти на отправку через `botManager` или корректно завершать временный `Bot`
  - [ ] Добавить `@grammyjs/transformer-throttler`
- **Зависимости**: Grammy
...
- **Тестирование**: ✅ `npx tsc --noEmit` и `pnpm build` проходят без ошибок
- **Результат**: 
