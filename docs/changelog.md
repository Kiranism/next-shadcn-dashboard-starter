...
### 🎯 Добавлено
- Пользовательская документация (user-docs): разделы «Интеграция с InSales» и «Интеграция с МойСклад» с пошаговыми инструкциями, проверкой и troubleshooting; добавлены в навигацию `_meta.ts` и на главную страницу.
- Новый раздел user-docs «Партнёрская программа (B2B)»: роли, многоуровневые комиссии, кабинет партнёра в Telegram, иерархия, уведомления.

### 🔄 Изменено (документация для пользователей)
- `bonus-system`: добавлены режимы расчёта SIMPLE/LEVELS, персональные планы комиссий, B2B-иерархия, FIFO-списание и срок действия, тип транзакции BIRTHDAY.
- `workflow-constructor`: добавлен раздел про триггеры по расписанию (день рождения, периодические сценарии).
- `admin-panel`: расширены интеграции (Tilda/InSales/МойСклад/MAX), иерархия партнёров, биллинг (автопродление, лимиты, ЮKassa).
- `faq`: вопросы про интеграции, партнёрскую программу и автоматизацию ко дню рождения.

### 🗑️ Удалено
- 86 мусорных файлов: 81 эфемерный отчёт/гайд из корня (`MOYSKLAD_*`, `INSALES_*`, `BONUS_*`, `PUSH_*`, `FIX_*`, `READY_*` и т.п.) + корневой JSON-дубль шаблона лояльности.
- 5 публичных тестовых страниц из `public/` (`test-*.html`), которые деплоились в прод.

### 🔄 Изменено
- Локализация интеграции InSales на русский: `metadata`, заголовок страницы, «Учётные данные API», «Домен магазина», fallback'и загрузки.
- Нативные `alert()` заменены на `toast` (sonner): `moysklad-direct/status-card.tsx` (тест подключения, синхронизация), `telegram-mailing-editor.tsx` (валидация).
- `accounts/page.tsx` приведён к дизайн-системе (`Heading` + `Separator` + glass-card + skeleton + empty state); `prompt()` сброса пароля заменён на диалог shadcn с валидацией.
- Все английские fallback'и `Loading...` локализованы на русский (dashboard, moysklad, moysklad-direct, insales, bonuses).
- Добавлена навигация «← Назад к интеграциям» на странице МойСклад (Loyalty API).

### 🐛 Исправлено (a11y)
- `insales/integration-form.tsx`: `<h1>` в середине формы → `<h3>` (иерархия заголовков).
- `insales/credentials.tsx`: связаны `<label>` с инпутами через `htmlFor`/`id`, добавлены `aria-label` на иконочные кнопки.
- `recent-projects.tsx`: кликабельное имя проекта `<p onClick>` → доступная `<button>`; `aria-label` на иконочную кнопку; действия видны при фокусе с клавиатуры (`focus-within`).
...
### 🔄 Изменено
- `tilda-bonus-widget.js` v2.9.15: модуль `referralAttribution` — захват `utm_ref` / `utm_org`, injection в формы Tilda.
- `gupil-attribution.js` — thin wrapper для обратной совместимости (делегирует виджету).
- Интеграция Tilda: убран отдельный «Шаг 2», достаточно одного скрипта виджета.
...
### 🎯 Добавлено
- Настройка `showPromocodeForGuests` для управления видимостью нативного поля промокода Tilda у неавторизованных клиентов.
- Чекбокс в настройках Tilda-виджета: «Показывать поле промокода без авторизации».
- Миграция БД `20260603192400_add_guest_promocode_widget_setting` с безопасным default `true`.

### 🔄 Изменено
- `public/tilda-bonus-widget.js` теперь скрывает или показывает поле промокода рядом с регистрационной плашкой по настройке проекта.
- `/api/projects/[id]/widget` отдаёт и сохраняет новую настройку вместе с остальными параметрами виджета.
