...
O `SidebarFooter` SHALL exibir um botão com ícone de sino (`bell`) acima do menu de avatar do usuário. O clique nesse botão SHALL abrir um `Sheet` lateral com a lista de notificações do usuário autenticado.
...
- **THEN** o Sheet SHALL exibir estado vazio com ícone e mensagem "Nenhuma notificação"
...
A query `GET /notifications` SHALL ser executada com `refetchInterval: 60_000` (1 minuto) enquanto o usuário estiver autenticado, independente do Sheet estar aberto ou fechado. O polling SHALL ser pausado quando a aba do navegador estiver em background.
...
O Sheet SHALL ser totalmente funcional em viewports a partir de 375px de largura. Em mobile, SHALL ocupar pelo menos 85% da largura da tela. Os botões de descarte SHALL ter área de toque mínima de 44×44px.
...
#### Scenario: Área de toque adequada
- **WHEN** o usuário toca o botão de descarte em dispositivo touch
- **THEN** a área de toque SHALL ser suficientemente grande para interação confortável (≥ 44px)
