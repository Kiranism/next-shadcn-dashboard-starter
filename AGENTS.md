...
- TanStack Form + Zod for form handling (`useForm` + shadcn `Field` primitives)
...
│   ├── forms/             # Form demos (shadcn TanStack Form conventions)
...
10. **Forms** - Use TanStack Form's `useForm` with shadcn `Field` primitives, following the official shadcn TanStack Form conventions (`form.Field` render props, `Field`/`FieldLabel`/`FieldError` anatomy, form-level Zod `onSubmit` validators). Never use `useState` inside `form.Field` render props — extract stateful controls into separate components.
